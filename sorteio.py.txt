...
df = df[df.iloc[:,1] != "SEM INFORMAÇÃO"]  # remove onde o nome é "SEM INFORMAÇÃO"
...
if st.button("Sortear"):
    ...
    for i, row in sorteados.iterrows():
        nome = row.iloc[1]   # segunda coluna = Nome
        endereco = row.iloc[2] if len(row) > 2 else ""
        if pd.notna(nome) and nome.strip() != "":
            st.markdown(
                f"<div style='background-color:#f77f00; padding:15px; margin:10px; border-radius:10px; text-align:center; color:white; font-size:22px; font-weight:bold;'>"
